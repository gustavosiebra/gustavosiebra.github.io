...
description: Neste post trago os passos para implementação do método, uma planilha e o código no MatLab para interpolação.
...
A forma amplamente difundida na bibliografia deve-se ao trabalho de Puls (1928), divulgado posteriormente de forma mais ampla como método de Puls modificado (BUTLER, 1957). 
...
Com base na duração do ramo ascendente do hidrograma de entrada, um período $$\Delta$$t será escolhido. Então, é feito uma combinação dos termos conhecidos para criar duas relações: gráficos de O versus 2S/$$\Delta$$t + O e versus de 2S/$$\Delta$$t – O (Figura \ref{fig:vazaoVersus2VolumeDeltat}) para achar a vazão de saída.
